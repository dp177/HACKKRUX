          ...
          <div className="grid lg:grid-cols-2">
            <aside className="relative hidden min-h-[620px] overflow-hidden bg-slate-900 p-8 lg:block">
              <div className="absolute inset-0">
                ...
                />
                <motion.div
                  animate={{ x: [0, -10, 14, 0], y: [0, 10, -8, 0] }}
                  transition={{ duration: 14, repeat: Infinity, ease: 'easeInOut' }}
                  className="absolute bottom-8 right-0 h-80 w-80 rounded-full bg-emerald-500/22 blur-3xl"
                />
                <div className="absolute inset-0 bg-[radial-gradient(circle_at_25%_20%,rgba(16,185,129,0.22),transparent_55%)]" />
              </div>
              ...
              <div className="relative z-10 flex h-full flex-col justify-between">
                <div>
                  ...
                  <div className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.12em] text-white/90">
                    <RiHospitalLine className="h-3.5 w-3.5" /> Hospital Operations
                  </div>
                  <h1 className="mt-6 max-w-md text-4xl font-semibold leading-tight tracking-tight text-white">
                    Command your hospital workflow from one secure portal
                  </h1>
                  <p className="mt-4 max-w-md text-sm leading-relaxed text-white/80">
                    Manage departments, onboard doctors, and monitor operations through a streamlined hospital control center.
                  </p>
                </div>

                <div className="space-y-3">
                  {[
                    { icon: RiShieldCheckLine, text: 'Role-based secure access' },
                    { icon: RiBarChartBoxLine, text: 'Live operations and team visibility' },
                    { icon: RiHeartPulseLine, text: 'Built for triage-first hospitals' }
                  ].map(({ icon: Icon, text }) => (
            ...
            <section className="p-6 sm:p-8 lg:p-10">
              <p className="mb-3 inline-block rounded-full bg-accent-100 px-3 py-1 text-xs font-semibold uppercase tracking-[0.14em] text-accent-800">Hospital Sign In</p>
              <h2 className="mb-2 text-3xl font-semibold tracking-tight text-slate-900">Welcome back</h2>
              <p className="mb-6 text-sm text-slate-600">Use approved credentials to access your hospital workspace.</p>
              ...
              <p className="mt-4 text-sm text-slate-600">
                Didn&apos;t have a hospital registered?{' '}
                <a href="/hospital-onboarding" className="font-semibold text-accent-700 hover:text-accent-800">
                  Onboard now
                </a>
              </p>

              <div className="mt-3 text-sm">
                <a href="/doctor-signin" className="font-semibold text-slate-600 hover:text-slate-700">Doctor Sign In</a>
              </div>
